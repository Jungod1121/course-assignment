...
从作业要求到 DOCX 报告的完整流水线。6 个阶段，不可跳过。
...
**确认**：作业标题、课程名称、输出目录。然后问学生信息（姓名/学号/班级/专业），**用户说先不填就用占位符**（`<!-- NAME -->`、`<姓名>`），后续用户自己替换。
默认输出目录：`<当前工作目录>/<作业简称>/`
创建目录后所有产物都在此处。
...
将 `report.html` 中的文本部分提交给 Humanizer-zh 扫描。Humanizer-zh 基于维基百科"AI 写作特征"指南，自动检测并修复：
- 夸大的象征意义和宣传性语言
- 以 -ing 结尾的肤浅分析
- 模糊的归因
- 破折号过度使用
...
Humanizer-zh 处理完后，再针对课程报告场景做专项检查：
