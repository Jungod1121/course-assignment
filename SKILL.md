...
### 5.5a Critic 自检（给用户看之前先自查）

HTML 写完但还没给用户看之前，**切换角色**——刚才你是 Actor（写报告的人），现在你是 Critic（挑刺的人）。逐项检查：

**任务完成度**：
- [ ] 作业要求的每个问题都覆盖了吗？有没有漏掉的子问题？
- [ ] 代码作业：代码运行结果和报告中引用的数据对得上吗？（对比 `code/output.txt` 和报告中的数据表格）
- [ ] 纯理论作业：每个论述点都有文献支撑吗？

**图表一致性**：
- [ ] 报告里引用的每张图（"如图 X 所示"），`figures/` 下面真的有那个文件吗？
- [ ] 图注和图表内容对得上吗？（图3标注的是 "ROC 曲线" 但图片是混淆矩阵？）

**文献可信度**：
- [ ] 每条参考文献都来自 Phase 4 的实际搜索结果吗？（不是凭记忆编的）
- [ ] 引用格式统一吗？（GB/T 7714 或 APA，不要混用）

**格式完整性**：
- [ ] 封面/目录/章节标题/正文/参考文献 —— 每个区域都不为空？
- [ ] 代码附录里的代码和 `code/` 目录下的实际文件一致吗？

**发现任何问题 → 回 Phase 5 修改，改完再自查一次。全部通过 → 进入 5.6 给用户看。**

这个步骤借鉴了两个做得好的竞品 skill 的共同设计：docx-course-report-writer 的强制双轮 Actor/Critic 流程，和 homework-solver 的独立 verification agent 验算。但我们的实现更轻——不依赖外部 agent 框架，靠"切换角色"做两遍检查。
